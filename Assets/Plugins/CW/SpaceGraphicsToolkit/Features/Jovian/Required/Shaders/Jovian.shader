...
#define unity_WorldToObject GetWorldToObjectMatrix()
	...
	{
		#if __SGT_OUTPUT
			float4 depth = tex2D(_SGT_DepthTex, d.texcoord0.xx); depth.a = saturate(depth.a + (1.0f - depth.a) * _SGT_Sky);
			float4 main  = _SGT_Color * _SGT_Brightness * ST_GetColor(d.texcoord1.xyz) * depth;
			float3 wcam  = _WorldSpaceCameraPos;
